...
for year in range(1999,2020):         
        for product_id, vender_id in my_list:
             ...
             #delete rows, in wich 'software type' is 'application'.
             dataframe.drop(dataframe.loc[dataframe['Software Type']== 'Application'].index, inplace=True)
             
             #order data by 'Publish Date' column
...
#for each pair (product_id, vender_id)
for product_id, vender_id in my_list:
    #this will extract all years for a product
    df_product=df[df.product_vender==str(product_id) + "_" + str(vender_id)]
    print(df_product.tail())
    writer = ExcelWriter('EachProducts/'+str(product_id) + "_" + str(vender_id)+'_for_Analysis.xlsx')
    df_product.to_excel(writer,'CVE Details',index=False)
    writer.save() 
